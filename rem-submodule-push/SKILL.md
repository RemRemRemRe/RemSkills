...
description: >
  ...
  submodules, or after `rem-submodule-sync` updated submodules.
metadata:
  category: workflow
  trigger: manual
---
...
discipline around pushing rewritten history. Whether the commits may be
...
Applies to refs you own — your fork's submodule branches, after a history you
deliberately rewrote. A shared project branch is never force-pushed; that
policy is owned by `rem-rewrite-commit-history` (Safety contract).

Only when the history is deliberately rewritten (user-requested cleanup),
and then:

- **Verify the tree first** — `git diff <pre-rewrite-tip> HEAD` must be **empty** (same resolution choices). The rule is owned by `rem-rewrite-commit-history`; a non-empty diff means content changed during the rewrite, so investigate before pushing anything.
