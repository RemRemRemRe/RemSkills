...
- **Never prune as part of a rewrite.** `git reflog expire`, `git gc --prune=now`,
  `filter-branch` / `filter-repo` cleanup, and deleting a branch, tag or stash that is the
  only reference to un-pushed work are irreversible and buy nothing here: the range check
  and the path-stripping check are defined by **refs**, not by dangling objects, and an
  object that only the reflog can reach is not history. Leave it to normal expiry —
  pruning replaces this skill's own safety net with nothing.
...
| Fold a correction across several target commits | split by file ownership, rebuild with `cherry-pick -n` + per-target `git apply --reject`, verify per commit — see `references/rebuild-playbook.md` §1 |
...
3. **Execute** — `git rebase -i <base>` (or `--autosquash` with `fixup!`
   ...
   Platform/tooling pitfalls (Windows CRLF and pipe corruption, `GIT_EDITOR`,
   temp-index rebuilds, gitlink ghosts) and the **mid-stack folding** recipe —
   assessment triage, the split-by-ownership steps, per-commit verification:
   `references/rebuild-playbook.md`.

4. **Verify** — first prove the content delta is the one you intended: a pure
   rewrite is a regroup, so `git diff <original-tip> <rewritten-branch>` must be
   empty (only grouping changed); when the same operation also carries a content
...
- [ ] No pruning ran as part of the rewrite (`reflog expire`, `gc --prune=now`, deleting the last reference to un-pushed work) — the reflog stays the recovery net
- [ ] Content delta is the one intended: empty for a pure rewrite, exactly the intended change when the rewrite carries edits; per-commit checks done during the rebuild (conflict markers, touched-file diffs)
