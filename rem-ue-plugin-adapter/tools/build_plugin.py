...
Usage:
    ...
    python build_plugin.py -n MyPlugin -v 5.7 5.8 --output-path "<output-dir>"
...
def load_engine_config(config_path: Path) -> Dict[str, Any]:
    if not config_path.exists():
        die(f"Config not found: {config_path}\n"
            f"  Create engines.local.json from the template "
            f"(see references/tools.md - 'engines.json (shared template)').")
...
def load_plugin_config(config_path: Path, plugin_name: str) -> Dict[str, Any]:
    ...
    if not config_path.exists():
        die(f"Plugin config not found: {config_path}\n"
            ...
            f"  Create one first (see references/tools.md - 'First-time setup').\n"
