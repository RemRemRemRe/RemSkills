...
5. **Verify the `REM_API` alias**
   ...
   correct uppercase module name (macro pattern:
   `rem-cpp-best-practices/references/modules.md` §15).
...
- [ ] Module is registered in the plugin's `.uplugin` `"Modules"` array (`rem-cpp-best-practices` §15)
- [ ] Build.cs dependencies include all required modules (`rem-cpp-best-practices` §15)

---

## Module file structure reference

The export macro pattern (`REM_API` aliased to `{NEWMODULE}_API`) is owned by
`rem-cpp-best-practices/references/modules.md` §15. Files created by the
template:
